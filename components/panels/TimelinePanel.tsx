...
const TimelinePanel: React.FC = () => {
  ...
  // 1. Logica per Registro Attività (Settimana Recente)
  const activityLog = useMemo(() => {
                ...
                if (task.completed && task.completedAt) {
                    ...
                    if (compDate >= oneWeekAgo) {
                        ...
                        const person = proj.people.find(p => p.id === assigneeId) || { 
                            id: 'unknown', name: 'Nessun Responsabile', initials: '?', color: 'bg-slate-400', version: 1 
                        };
    ...
    // Ordina giorni in ordine decrescente
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 z-20 flex-shrink-0">
          <div className="flex items-center gap-6">
              ...
              <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                  ...
                  >
                      <History className="w-3.5 h-3.5" /> ATTIVITÀ
                  </button>
              </div>
          </div>
                                  ...
                                  return (
                                      <div key={`${branch.projectId}-${branch.id}`} className="h-12 border-b border-transparent relative group">
                                          ...
                                          {dueX !== null && (
                                            ...
                                            >
                                                <div className="hidden group-hover/marker:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-800 text-white text-[9px] rounded whitespace-nowrap z-50 shadow-lg pointer-events-none">
                  ...
                  <div className="max-w-4xl mx-auto space-y-12">
                      {activityLog.length === 0 && (
                          <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                              ...
                              <p className="text-lg font-bold">Nessuna attività recente</p>
                              <p className="text-sm">Inizia a completare i task per vedere il log qui.</p>
                          </div>
                      )}
                                          ...
                                          <div className="grid grid-cols-1 gap-2">
                                              {group.tasks.map(task => (
                                                  <div 
                                                    ...
                                                    className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 hover:border-indigo-300 transition-all cursor-pointer group"
                                                  >
